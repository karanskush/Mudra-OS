...
const EndpointList = ({ endpoints, onSelect, selectedKey }: { endpoints: Endpoint[], onSelect: (key: string) => void, selectedKey: string }) => (
  <div className="space-y-2">
    {endpoints.map(ep => (
      ...
      >
        <span className={`font-mono text-xs px-2 py-1 rounded ${ep.method === 'GET' ? 'bg-green-100 text-green-700' : ep.method === 'POST' ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700'}`}>{ep.method}</span>
