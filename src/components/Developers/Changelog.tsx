...
const Changelog: React.FC = () => (
  <section className="mb-8 p-6 bg-gradient-to-r from-gray-50 to-blue-50 dark:from-slate-900 dark:to-blue-900 rounded-2xl shadow border border-gray-200 dark:border-blue-800">
    <h2 className="text-2xl font-bold mb-2 flex items-center gap-2">
      <History className="w-6 h-6 text-blue-600 dark:text-blue-300" /> API Changelog
    </h2>
    <ul className="space-y-2">
      {changelog.map(entry => (
        <li key={entry.version} className="border-l-4 border-blue-400 pl-4 py-2">
          <div className="flex items-center gap-2">
            <span className="font-mono text-blue-700 dark:text-blue-300">{entry.version}</span>
            <span className="text-xs text-gray-500 dark:text-gray-400">({entry.date})</span>
          </div>
          <div className="text-gray-700 dark:text-gray-200 text-sm">{entry.notes}</div>
        </li>
      ))}
    </ul>
  </section>
