...
const FeaturePill = ({ icon: Icon, text }: { icon: any; text: string }) => (
  <div
    className="flex items-center gap-2 bg-white/70   border border-gray-200/50  rounded-full px-4 py-2 text-sm font-medium text-gray-700  shadow hover:shadow-md transition-all duration-200 hover:scale-105"
  >
...
const MetricCard = ({ value, label, trend }: { value: string; label: string; trend: string }) => (
  <motion.div
    ...
    className="bg-gradient-to-br from-blue-50 to-indigo-50   rounded-xl p-4 border border-blue-100  shadow-md hover:shadow-lg transition"
  >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="flex justify-center mb-8">
            <div className="inline-flex items-center gap-2 bg-white/80   border border-blue-200/50  rounded-full px-6 py-3 text-sm font-medium text-gray-700  shadow-lg">
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div className="text-center lg:text-left">
              ...
              <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start mb-8">
                <motion.button
                  ...
                  className="group relative bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-pink-600 text-primary px-8 py-4 rounded-xl text-lg font-semibold shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
                  whileHover={{ scale: 1.04 }}
                  whileTap={{ scale: 0.98 }}
                >
                  <span>Start Building</span>
                  <ArrowRight className="h-5 w-5 group-hover:translate-x-1 transition-transform duration-300" />
                  <div className="absolute inset-0 bg-surface rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
                </motion.button>
                <motion.button
                  className="group bg-white/80   hover:bg-white :bg-white text-gray-900  px-8 py-4 rounded-xl text-lg font-semibold border border-gray-200/50  hover:border-gray-300 :border-outline-variant flex items-center justify-center gap-2 shadow focus:outline-none focus:ring-2 focus:ring-blue-200"
              ...
              {/* --- Social Proof --- */}
              <motion.div variants={itemVariants} className="flex items-center gap-4 justify-center lg:justify-start text-sm text-gray-600 ">
                <div className="flex items-center gap-1">
                  <div className="flex">
                    {[...Array(5)].map((_, i) => (
            ...
            <motion.div variants={floatingCardVariants} className="relative">
              <div className="relative bg-white/80   rounded-3xl p-8 shadow-2xl border border-gray-200/50 ">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-bold text-gray-900 ">MudraCore OS Dashboard</h3>
                  ...
                  {[
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        <div className={`text-xs ${transaction.status === 'Completed' ? 'text-secondary ' : 'text-yellow-600 '}`}>{transaction.status}</div>
                      </div>
                    </motion.div>
                  ))}
                </div>
              ...
              <motion.div
                ...
                className="absolute -bottom-6 -right-6 bg-white  rounded-2xl p-4 shadow-xl border border-gray-200  "
              >
                <div className="text-xs text-gray-500  mb-2">API Response</div>
                <div className="font-mono text-xs text-secondary ">
                  <div>POST /v1/payments</div>
                  <div className="text-gray-600 ">201 Created</div>
                  <div className="text-gray-400 ">~45ms</div>
                </div>
              </motion.div>
              ...
              <motion.div
                ...
                className="absolute -top-4 -left-4 bg-brand-500 text-primary rounded-full p-3 shadow-lg"
              >
          ...
          <motion.div variants={itemVariants} className="mt-20">
            <div className="text-center mb-8">
              <p className="text-sm text-gray-500  font-medium">Trusted by leading fintech companies worldwide</p>
            </div>
            <div className="relative overflow-hidden">
              ...
              >
                {[...logoMarqueeData, ...logoMarqueeData].map((logo, idx) => (
                  <div
                    key={idx}
                    className="text-2xl font-bold text-gray-400  hover:text-blue-600 :text-blue-400 transition-colors duration-300 grayscale hover:grayscale-0"
                  >
                    {logo}
                  </div>
                ))}
